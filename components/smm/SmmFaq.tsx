...
export type SmmFaqItem = {
    question: string;
    answer: string;
};

export default function SmmFaq({ items }: { items: SmmFaqItem[] }) {
    const [openIndex, setOpenIndex] = useState(0);

    return (
        <section id="sss" aria-labelledby="smm-faq-heading" className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            <h2 id="smm-faq-heading" className="text-2xl font-black tracking-tight text-slate-950">
                Sıkça Sorulan Sorular
            </h2>
            <div className="mt-5 divide-y divide-slate-200">
                {items.map((item, index) => {
                    const isOpen = openIndex === index;
                    const buttonId = `smm-sss-${index}-btn`;
                    const panelId = `smm-sss-${index}-content`;
