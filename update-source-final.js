...
for (const config of regexMap) {
  ...
  // replace title.tr
  let contentSlice = content.substring(seoIdx, seoIdx + 3000); 
  ...
  content = content.substring(0, seoIdx) + chunk + content.substring(seoIdx + 3000);
  ...
  topChunk = replaceProperty(topChunk, 'shortDescription: { tr: ', ', en: ', '"' + config.description.replace(/"/g, '\\\\"') + '"'); 
